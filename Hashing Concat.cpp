...
int main()
{
    ...
    for (int cs=1; cs<=t; ++cs)
    {
        ...
        for(int i=1; i+2<n; i++)
        {
            PLL Left = substringHash(ha, 1, i);
            PLL Right = substringHash(ha, i+3, n);
            PLL combined = concat(Left, Right, n-i-2);
            if(!cnt.count(combined))
            {
                cnt[combined] = 1;
                ans++;
            }
        }
        PLL L = substringHash(ha, 3, n);
        PLL R = substringHash(ha, 1, n-2);
        if(!cnt.count(L)) ans++;
        if(!cnt.count(R)) ans++;
        printf("%d\n",ans);
    }
